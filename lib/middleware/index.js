...
RedisSessions.prototype.createWithToken = function createWithToken (options, cb) {
    ...
    options = this._validate(options, ["app", "id", "token", "ip", "ttl", "d", "no_resave"], cb);
    if (options === false) {
      return;
    }
    token = options.token;
...
module.exports = function (session) {
  ...
  const prepareSession = Store.createSession;
  ...
  RedisSessionsStore.prototype.set = function (sid, sess, fn) {
    ...
    store.rs.get({ app: store.appname, token: sid }, function (err, data) {
      ...
      if (!data || !data.token) {
        ...
        return store.rs.set(datum, function (er, resp) {
          if (er) return fn(er);
          debug('SET complete');
