...
function mount (app, passport) {
  ...
  function suggest (req, res, next) {
    ...
    if (req.body && req.body.remove_credentials == "bridge") {
      settings.ok = 1;
  ...
  function enable (req, res, next) {
    ...
    if (res.locals.attempt && res.locals.attempt.ok) {
      ...
      payload.ENABLE = (res.locals.attempt.ok && res.locals.attempt.settings.mode != 'delete' ? 
      ...
      if (res.locals.attempt.settings.mode == 'delete') {
        payload.BRIDGE_USER_NAME = '';
        payload.BRIDGE_PASSWORD = '';
      }
  ...
  function verify (req, res, next) {
    ...
    if (!req.bridge.enabled && ([null, 'verify', 'delete'].indexOf(req.bridge.mode) < 1)) {
      return next( );
    }
    if ( req.bridge.mode == 'verify'  && ( req.bridge.creds.accountName.length < 3
